...
public class BST<T extends Comparable<? super T>> implements Iterable<T>{
   ...
   private class Node implements BSTNode<T> {
	...
	private class Whatever implements Iterator<T>
	{
		private SimpleArrayStack<Node> as = new SimpleArrayStack<Node>();
		...
		public Whatever(Node root)
		{
			start = root;
			while(true)
			{
				as.push(start);
				if(start.left != EMPTY_NODE)
				{
		...
		{				
			...
			Node rightOne = null;
			Node popped = as.pop();
			T returnee = popped.element;
			
			if(popped.right != EMPTY_NODE)
			{
				rightOne = (Node)popped.right;
				while(true)
				{
					as.push(rightOne);
					if(rightOne.left != EMPTY_NODE)
					{
						rightOne= (Node) rightOne.left;
					}
					else
					{
						break;
	...
	public Iterator<T> iterator()
	{
		...
		{
			whatever = new Whatever((Node)root);
		}
		return whatever;
	}
